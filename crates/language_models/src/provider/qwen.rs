...
const PROVIDER_NAME: LanguageModelProviderName = LanguageModelProviderName::new("Qwen AI");

#[derive(Default, Clone, Debug, PartialEq)]
pub struct QwenSettings {
    pub api_url: String,
    /// Extend Zed's list of Qwen AI models.
...
const API_KEY_ENV_VAR_NAME: &str = "QWEN_API_KEY";
...
impl LanguageModelProvider for QwenLanguageModelProvider {
    ...
    fn default_model(&self, _cx: &App) -> Option<Arc<dyn LanguageModel>> {
        Some(self.create_language_model(QwenModel::default()))
    }

    fn default_fast_model(&self, _cx: &App) -> Option<Arc<dyn LanguageModel>> {
        Some(self.create_language_model(QwenModel::default_fast()))
    }
    ...
    fn provided_models(&self, cx: &App) -> Vec<Arc<dyn LanguageModel>> {
        ...
        // Add base models
        for model in QwenModel::all_models() {
            if !matches!(model, QwenModel::Custom { .. }) {
                models.insert(model.name().to_string(), model);
            }
        }

        // Override with available models from settings
        for model in &QwenLanguageModelProvider::settings(cx).available_models {
            models.insert(
                model.name.clone(),
                QwenModel::Custom {
...
impl QwenModel {
    ...
    pub fn max_token_count(&self) -> u64 {
        match self {
            Self::Qwen3Code => 128_000,
            Self::QwenMax => 30_720,
            Self::QwenPlus => 128_000,
            Self::QwenTurbo => 8_192,
...
impl QwenModelInstance {
    fn stream_completion(
        ...
        'static,
        Result<
            BoxStream<'static, Result<QwenResponseEvent, QwenError>>,
...
impl From<http_client::http::Error> for QwenError {
    fn from(error: http_client::http::Error) -> Self {
        QwenError::Other(anyhow::Error::from(error))
    }
}

// Convert Zed LanguageModelRequest to Qwen request
fn into_qwen(
...
// Stream completion function for Qwen API
async fn stream_completion(
    http_client: &dyn HttpClient,
    api_url: &str,
    api_key: &str,
    request: QwenRequest,
) -> Result<impl Stream<Item = Result<QwenResponseEvent, QwenError>> + use<>, QwenError> {
    use http_body_util::BodyExt;
    use http_client::http::{Method};
    use http_client::AsyncBody;
    
    let json_request = serde_json::to_string(&request)
        .map_err(|e| QwenError::Other(anyhow::Error::from(e)))?;
    ...
    let response = http_client.send(http_request?).await.map_err(|e| QwenError::Other(e.into()))?;

    if !response.status().is_success() {
        return Err(QwenError::HttpError {
    ...
    let processed_stream = lines
        .filter_map(|line| async move {
            match line {
                Ok(line) => {
                    ...
                    if let Some(json_str) = line.strip_prefix("data: ") {
                        if json_str.trim() == "[DONE]" {
                            return Some(Ok(QwenResponseEvent::Done));
                        }
                        
                        // Try to parse the response
                        if let Ok(response_chunk) = serde_json::from_str::<QwenResponse>(json_str) {
                            if let Some(choice) = response_chunk.choices.first() {
                                if let Some(content) = &choice.delta.content {
                                    return Some(Ok(QwenResponseEvent::Content(content.clone())));
                ...
                Err(e) => Some(Err(QwenError::Other(anyhow::anyhow!("Failed to read line: {}", e)))),
            }
        });
...
impl LanguageModel for QwenModelInstance {
    ...
    fn supports_tools(&self) -> bool {
        matches!(self.model, QwenModel::QwenMax | QwenModel::QwenPlus | QwenModel::Custom { .. })
    }

    fn supports_images(&self) -> bool {
        false // Basic Qwen models don't support images in this implementation
    }
    ...
    fn count_tokens(
        ...
        // Using a fallback token counting method for Qwen
    ...
    > {
        ...
        let future = self.request_limiter.stream(async move {
            ...
            Ok(QwenEventMapper::new().map_stream(response))
        });
...
impl QwenEventMapper {
    ...
    pub fn map_stream(
        mut self,
        events: Pin<Box<dyn Send + Stream<Item = Result<QwenResponseEvent, QwenError>>>>,
    ...
    pub fn map_event(
        &mut self,
        event: QwenResponseEvent,
    ) -> Vec<Result<LanguageModelCompletionEvent, LanguageModelCompletionError>> {
        match event {
            QwenResponseEvent::Content(text) => {
                vec![Ok(LanguageModelCompletionEvent::Text(text))]
            }
            QwenResponseEvent::Done => {
...
impl From<QwenError> for LanguageModelCompletionError {
    fn from(error: QwenError) -> Self {
        use http_client::http::StatusCode;
        match error {
            QwenError::ApiError { message } => LanguageModelCompletionError::AuthenticationError {
                provider: PROVIDER_NAME,
                message,
            },
            QwenError::HttpError { status } => {
            ...
            QwenError::Other(error) => LanguageModelCompletionError::Other(error),
        }
    }
}
...
impl ConfigurationView {
    const PLACEHOLDER_TEXT: &'static str = "your-qwen-api-key-here";
        ...
        if self.load_credentials_task.is_some() {
            ...
            v_flex()
                ...
                .child(Label::new(format!("To use {}, you need to add an API key. Follow these steps:", match &self.target_agent {
                    ConfigurationViewTargetAgent::ZedAgent => "Zed's agent with Qwen AI".into(),
